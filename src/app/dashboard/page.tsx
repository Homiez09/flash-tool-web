...
export default function DashboardPage() {
  ...
  const handleFlash = async () => {
    if (!sysConfig.features.flash) { toast.error("ฟีเจอร์นี้ถูกปิดใช้งานชั่วคราว"); return; }
    ...
    addLog(`Starting Flash Engine... Partition: ${partition}`);
    ...
    try {
      ...
      await fetch("/api/user/use-credits", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ amount: cost, description: `Flash ${partition} (${flashMethod})` }),
      });
      await update();
      fetchHistory();
    ...
    <div className="flex min-h-screen bg-gray-50/50 font-sans tracking-tight">
      ...
      <aside className="hidden md:flex w-64 flex-col bg-white border-r sticky top-0 h-screen shadow-sm">
          ...
          {menuItems.map((item) => (
            <button
              ...
              className={cn(
                ...
                activeTab === item.id ? "bg-blue-600 text-white shadow-xl" : "text-gray-400 hover:bg-gray-50 hover:text-gray-900"
              )}
            >
        ...
        <div className="p-4 border-t border-gray-100 space-y-4">
          ...
          <Button variant="ghost" className="w-full justify-start text-red-500 rounded-xl font-black text-[10px] uppercase tracking-widest cursor-pointer" onClick={() => signOut()}>
                  ...
                  <Card className="bg-gray-950 text-white border-none shadow-2xl overflow-hidden rounded-[2.5rem] ring-4 ring-white/5">
                    <CardHeader className="bg-gray-900/80 py-5 px-8 border-b border-white/5 flex items-center justify-between text-gray-900">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => setLogs([])} className="p-2 text-gray-500 hover:text-red-500 cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </CardHeader>
                    <CardContent className="p-8 h-80 overflow-y-auto font-mono text-[11px] space-y-2 bg-black/40">
                      {logs.map((log, i) => (
                        <p key={i} className={cn(log.includes("Error") || log.includes("Failed") ? "text-red-400" : log.includes("Success") ? "text-green-400" : "text-blue-300")}>
                          <span className="text-white/10 mr-3">➜</span>{log}
                        </p>
                      ))}
                    </CardContent>
                  </Card>
                ...
                <Card className="lg:col-span-2 border-2 border-blue-50 shadow-2xl rounded-[2.5rem] overflow-hidden bg-white">
                   <CardHeader className="bg-gray-50/50 border-b p-8">
                    <div className="flex gap-4">
                       ...
                       <button onClick={() => setFlashMethod("url")} className={cn("flex-1 h-14 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-3 border-4 transition-all cursor-pointer", flashMethod === "url" ? "bg-blue-600 text-white border-blue-600 shadow-xl" : "bg-white text-gray-300 border-gray-100")}>
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {historyData.map((item) => (
                          <tr key={item.id} className="hover:bg-slate-50 transition-all">
                            <td className="px-12 py-8 text-[11px] font-black text-slate-500 uppercase">{new Date(item.createdAt).toLocaleString()}</td>
                            <td className="px-12 py-8">
                              <span className={cn("px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-widest ring-2 ring-inset", item.type === "ADD" ? "bg-green-50 text-green-700 ring-green-100" : "bg-red-50 text-red-700 ring-red-100")}>{item.type}</span>
                            </td>
                            <td className="px-12 py-8 text-xs font-black text-slate-900 uppercase tracking-tight">{item.description}</td>
                            <td className={cn("px-12 py-8 text-right font-black text-base uppercase tracking-tighter", item.type === "ADD" ? "text-green-600" : "text-red-600")}>{item.type === "ADD" ? "+" : "-"}{item.amount} <span className="text-[10px] opacity-40 ml-1">c</span></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            <div className="max-w-2xl mx-auto space-y-10 animate-in slide-in-from-bottom-6 duration-700 mt-20">
               <Card className="rounded-[3rem] border-4 border-red-50 bg-red-50/10 overflow-hidden shadow-2xl shadow-red-600/5">
                  ...
                  <CardContent className="p-12 pt-0">
                     <Button variant="destructive" className="w-full h-16 rounded-2xl font-black text-lg uppercase tracking-tight gap-4 cursor-pointer shadow-2xl shadow-red-600/20 active:scale-95 transition-all" onClick={() => signOut()}><LogOut className="w-6 h-6" /> Sign Out Now</Button>
                  </CardContent>
               </Card>
            </div>
          )}
